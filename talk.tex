...
\begin{frame}
	...
	\title{
		...
		\vspace*{-.7cm}}
	\author{Jonathan Wenger\vspace{-1cm}} \date{Group Meeting MoML\\ September 8, 2020}
...
\begin{frame}\frametitle{Why should I care about PN Methods?}
	\framesubtitle{}

	\emph{Theoretical Appeal}
	\begin{itemize}
		\item enriched \textit{interpretation of classical methods}
		\item formalizes the \textit{design of adaptive methods} using tools from decision theory
	\end{itemize}

	\emph{Practical Advantages}
	\begin{itemize}
		\item allows to build \textit{customized methods} for specific problems with bespoke priors
		\item provides a way of \textit{setting parameters of numerical methods} via the Bayesian formalism
		\item expedites the \textit{solution of mutually related problems} of similar type
		\item naturally incorporates \textit{sources of stochasticity} in the computation
		\item \textit{structural uncertainty} via a probability measure compared to an error estimate
	\end{itemize}

	\emph{Vision}
	\begin{itemize}
		\item \textit{propagation of uncertainty} through chains of computations
	\end{itemize}

\end{frame}

\begin{frame}\frametitle{Components of a PN Method}
	\framesubtitle{}
...
\begin{frame}\frametitle{But what \textit{exactly} is a PN Method?}
	\framesubtitle{A formal definition.}
	...
	\begin{columns}[totalwidth=\textwidth]
		...
		\begin{algorithm}[H]
			...
			\begin{algorithmic}[1]
				...
				\State \(\rvq \gets \textsc{BeliefUpdate}((\vs, \vy), \rvx, F, {\color{gray}\theta_*})\)
				% \Comment{posterior}
				\EndWhile
				\State \Return $\rvq$
				\EndProcedure
